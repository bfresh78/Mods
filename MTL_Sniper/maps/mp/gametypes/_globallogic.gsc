...
init() {
    ...
    thread maps\mp\gametypes\_killratio::main(); 	 
}
registerDvars() {
...
endGame(winner, endReasonText) {
    ...
    if ((level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd) {
        ...
        if (isDefined(level.pers_killcam) && endReasonText != game["strings"]["time_limit_reached"]) {
	    song=(1+randomInt(20));
            players = getEntArray("player", "classname");
            for (i = 0; i < players.size; i++) {
                players[i] notify("reset_outcome");
                players[i] thread maps\mp\gametypes\_killcam::roundwinningkill(level.pers_killcam["lpattacknum"], level.pers_killcam["killcamentity"], level.pers_killcam["sWeapon"], undefined, level.pers_killcam["attacker"], level.pers_killcam["self"]);
				players[i] playLocalSound("endround"+song);
            }
            wait 3.5;
            setDvar("timescale", 1);
            wait 2.5;
...
menuAutoAssign() {
    ...
    if (self.pers["team"] == "allies" && oldTeam != self.pers["team"]) {
        ...
        else iPrintLN(self.name + " ^1Joined ^0Defence^7....");
        ...
        else iPrintLN(self.name + " ^1Joined ^0Attack^7...");
    }
    for (i = 0; i < level.players.size; i++) if (level.players[i].pers["team"] == "spectator") level.players[i] thread promod\shoutcast::resetShoutcast();
...
timeLimitClock() {
    ...
    while (isDefined(game["state"]) && game["state"] == "playing") {
        if (!level.timerStopped && level.timeLimit) {
            ...
            if (timeLeftInt >= 30 && timeLeftInt <= 60) level notify("match_ending_soon");
    ...
    if (isDefined(game["PROMOD_KNIFEROUND"]) && game["PROMOD_KNIFEROUND"]) {
        for (i = 0; i < level.players.size; i++) {
            ...
            if ((player.pers["team"] == "allies" || player.pers["team"] == "axis") && isDefined(player.pers["class"])) {
                attachment = "";
                if (player.pers[player.pers["class"]]["loadout_secondary_attachment"] == "silencer") attachment = "_silencer";
...
Callback_StartGameType() {
    ...
    if (!isDefined(game["gamestarted"])) {
        ...
        if (game["attackers"] == "allies" && game["defenders"] == "axis") {
            game["strings"]["allies_name"] = "^1Attack";
            game["strings"]["axis_name"] = "^0Defence";
            game["strings"]["allies_eliminated"] = "^1Attack eliminated";
            game["strings"]["axis_eliminated"] = "^0Defence eliminated";
            game["strings"]["allies_forfeited"] = "^1Attack forfeited";
            game["strings"]["axis_forfeited"] = "^0Defence forfeited";
        } else {
            game["strings"]["allies_name"] = "^0Defence";
            game["strings"]["axis_name"] = "^1Attack";
            game["strings"]["allies_eliminated"] = "^0Defence eliminated";
            game["strings"]["axis_eliminated"] = "^1Attack eliminated";
            game["strings"]["allies_forfeited"] = "^0Defence forfeited";
            game["strings"]["axis_forfeited"] = "^1Attack forfeited";
        }
        switch (game["allies"]) {
            case "sas":
                ...
                game["colors"]["allies"] = (0.255, 0.0, 0.0);
        ...
        switch (game["axis"]) {
            case "russian":
                ...
                game["colors"]["axis"] = (0.255, 0.0, 0.0);
    ...
    if ((isDefined(game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "match" || getDvarInt("promod_allow_readyup") && isDefined(game["CUSTOM_MODE"]) && game["CUSTOM_MODE"]) && !game["roundsplayed"] && !game["promod_first_readyup_done"]) game["promod_do_readyup"] = true;
    ...
    thread zaggorec\_general::init();  	     
    thread zaggorec2\_ads::init();
    thread zaggorec2\_fov::main();
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime) {
    ...
    if (!(iDFlags & level.iDFLAGS_NO_PROTECTION)) {
        ...
        if (isDefined(eAttacker) && eAttacker != self) {
            ...
            hasBodyArmor = false;
            if (iDamage > 0 && getDvarInt("scr_enable_hiticon") == 1) eAttacker thread maps\mp\gametypes\_damagefeedback::updateDamageFeedback(hasBodyArmor);
            else if (iDamage > 0 && getDvarInt("scr_enable_hiticon") == 2 && !(iDFlags & level.iDFLAGS_PENETRATION)) eAttacker thread maps\mp\gametypes\_damagefeedback::updateDamageFeedback(hasBodyArmor);
        }
    ...
    if (isDefined(eAttacker) && isPlayer(eAttacker)) {
        if (eAttacker != self && (level.rdyup || isDefined(game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "strat") && isDefined(sHitLoc)) {
            if (sHitLoc == "none") {
                eAttacker iprintln("You inflicted " + "^1" + iDamage + "^7 damage to " + self.name);
        ...
        } else if (eAttacker == self && (level.rdyup || isDefined(game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "strat") && isDefined(sHitLoc) && sHitLoc == "none") self iprintln("You inflicted ^1" + iDamage + "^7 damage to yourself");
    } else if ((level.rdyup || isDefined(game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "strat") && sMeansOfDeath == "MOD_FALLING") self iprintln("You inflicted ^1" + iDamage + "^7 damage to yourself");
    ...
    if (!isDefined(self.switching_teams)) self notify("updateshoutcast");
}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration) {
    ...
    if ((!isDefined(level.strat_over) || level.strat_over) && (!isDefined(game["promod_do_readyup"]) || !game["promod_do_readyup"])) self maps\mp\gametypes\_weapons::dropWeaponForDeath(attacker);
    self.sessionstate = "dead";
    ...
    if (isDefined(attacker) && isPlayer(attacker) && attacker != self) {
        if (!isDefined(level.pers_killcam)) level.pers_killcam = [];
    ...
    } else if (isDefined(attacker) && !isPlayer(attacker) && attacker != self) {
        if (!isDefined(level.pers_killcam)) level.pers_killcam = [];
        level.pers_killcam["lpattacknum"] = self getEntityNumber();
        level.pers_killcam["killcamentity"] = self getEntityNumber();
        level.pers_killcam["sWeapon"] = sWeapon;
        level.pers_killcam["attacker"] = attacker;
    }
    wait 0.25;
